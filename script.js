...
function loadRecaptcha() {
    ...
    script.src = 'https://www.google.com/recaptcha/enterprise.js?render=explicit&onload=initRecaptcha';
    script.async = true;
    script.defer = true;
...
function attachRecaptchaTriggers() {
    ...
    const scrollHandler = () => {
        if (!isRecaptchaLoaded && (window.scrollY + window.innerHeight > document.documentElement.scrollHeight - 1000)) {
            loadRecaptcha();
...
window.initRecaptcha = function() {
    ...
    if (thankyouContainer) {
        thankyouRecaptchaWidgetId = grecaptcha.enterprise.render('recaptcha-thankyou', {
            'sitekey': RECAPTCHA_SITE_KEY,
            'action': 'CONTACT_FORM_SUBMIT'
        });
    }
...
if (thankyouSendBtn) {
    thankyouSendBtn.addEventListener('click', function(e) {
        e.preventDefault();
        e.stopPropagation();
        handleFormSubmit('thankyouEmailInput', 'thankyouSendBtn', thankyouRecaptchaWidgetId);
    });
...
function openPopup(webUrl, androidUrl, iosUrl) {
    ...
    if (validOptions === 1 && targetUrl) {
        ...
        countdown.innerHTML = 'Redirecting in <span id="countdown-timer">5</span> seconds...';
...
document.addEventListener('keydown', (e) => {
    if (e.key === 'Escape' && projectPopupOverlay && projectPopupOverlay.classList.contains('active')) {
        closePopup();
    }
});
